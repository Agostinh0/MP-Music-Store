...
public class RepositorioFuncionarios implements IRepositorioFuncionarios, Serializable{
	...
	private static final long serialVersionUID = -4071201937008428791L;
	...
	public void cadastrar(String nome, long cpf, String endereco, long tel, float salario, 
			int numContrato, Usuario usuario, TipoFuncionario tipoFunc){
		Funcionario f = new Funcionario(nome, cpf, endereco, tel, salario, numContrato, usuario, tipoFunc);
		this.cadastrar(f);
	}
	...
	public boolean loginCheck(Usuario u) {
		boolean resultado = false;
		for(int i=0; i < listaFuncs.size(); i++) {
			if(this.listaFuncs.get(i) instanceof Funcionario) {
				if (this.listaFuncs.get(i).getUsuario().equals(u)) {
					resultado =  true;
				}
			}
		}
	...
	public boolean admLoginCheck(Usuario u) {
		boolean resultado = false;
		for(int i=0; i < listaFuncs.size(); i++) {
			if(this.listaFuncs.get(i) instanceof Funcionario) {
				if (this.listaFuncs.get(i).getUsuario().equals(u) && this.listaFuncs.get(i).isAdm()) {
					resultado =  true;
				}
			}
		}
